...
try:    from .core import synchronized, Primitive
except: from core import synchronized, Primitive
...
if __name__ == '__main__':
    import sys
    if sys.argv[1:] and sys.argv[1] == "gen":
        import random
        for _ in range(100):
        ...
        p = ShellCommand("python %s gen" % (__file__,))
        status = None
        while status is None:
            status, out, err = p.wait(1.0)
            print "%s out:[%s] err:[%s]" % (status, out, err)
